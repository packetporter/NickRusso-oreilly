...
def get_triangles(filename):
    """
    Read in from the YAML file supplied and create
    a list of triangle based on the input data.
    """
    try:
        handle = open(filename, "r")
        data = yaml.safe_load(handle)
    ...
    triangle_objects = []
    for sides in data["triangle_list"]:
        for _, value in sides.items():
            side_1, side_2, side_3 = value
            triangle_objects.append(Triangle(side_1, side_2, side_3))
    # Return the list of Circle objects
